...
/**
 * ============================================================
 *  FILE: src/rekrutmen/DatabasePelamar.vue
 *  TUJUAN: Tampilan database pelamar kerja - data dari API.
 *
 *  ✅ UNTUK BACKEND: File ini TIDAK perlu disentuh. Ubah endpoint
 *  di useDatabasePelamar composable.
 *  ✅ UNTUK FRONTEND: Hanya ubah tampilan (HTML/CSS) di sini.
 * ============================================================
 */

import {
  Users, UserSearch, UserCheck, UserX,
  Search, ChevronDown, Loader2,
  AlertCircle, RefreshCw, ArrowRight,
} from 'lucide-vue-next'

import AppLayout from '@/components/AppLayout.vue'
import { useDatabasePelamar } from '@/composables/useDatabasePelamar'
...
const statIcons  = [Users, UserSearch, UserCheck, UserX]
const statColors = [
...
const tahapBadge = {
  Screening: 'badge-slate',
  Interview: 'badge-amber',
  Diterima: 'badge-green',
  Ditolak: 'badge-red',
}

const nextTahap = { Screening: 'Interview', Interview: 'Diterima' }
</script>

<template>
  <AppLayout breadcrumb="Rekrutmen" title="Database Pelamar">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center min-h-[400px] gap-4">
      ...
      <p class="text-slate-500 text-sm font-black tracking-wide">Memuat data pelamar...</p>
    </div>
    ...
    <div v-else class="space-y-5 pb-6 animate-fadein">
      ...
      <!-- ── Tabel Database Pelamar ── -->
      <div class="card rounded-[24px] p-5 sm:p-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <div>
            <h3 class="card-title">Daftar Pelamar Kerja</h3>
            <p class="card-sub">{{ filtered.length }} pelamar ditemukan</p>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-2">
            <div class="relative">
              ...
              <input
                ...
                class="h-9 w-56 rounded-xl border-2 border-slate-200 bg-white pl-9 pr-3 text-sm font-bold text-slate-700 outline-none focus:border-sky-400 transition-all placeholder:font-normal placeholder:text-slate-400"
              />
            </div>
            <div class="relative">
              <select
              ...
              >
                <option v-for="s in tahapList" :key="s">{{ s }}</option>
              </select>
        ...
        <div class="overflow-x-auto -mx-1">
          <table class="w-full min-w-[680px]">
            <thead>
              <tr>
                <th class="th">Pelamar</th>
                <th class="th">Posisi Dilamar</th>
                <th class="th">Tanggal Lamar</th>
                <th class="th">Tahap</th>
                <th class="th"></th>
              </tr>
            </thead>
            <tbody>
              ...
              >
                ...
                <td class="td text-sm font-bold text-slate-600">{{ item.posisi }}</td>
                <td class="td text-sm font-bold text-slate-600 tabular-nums">{{ item.tanggal }}</td>
                <td class="td">
                  <span class="badge" :class="tahapBadge[item.tahap]">{{ item.tahap }}</span>
                </td>
                <td class="td">
                  <button
                    v-if="nextTahap[item.tahap]"
                    @click="ubahTahap(item, nextTahap[item.tahap])"
                    :disabled="updating === item.id"
                    class="lanjut-btn flex items-center gap-1.5 px-3.5 py-2 rounded-xl font-black text-xs transition-all duration-200 hover:-translate-y-0.5 active:scale-95 border-2 disabled:opacity-60"
                  >
                    <Loader2 v-if="updating === item.id" :size="13" class="animate-spin" />
                    <ArrowRight v-else :size="13" />
                    Ke {{ nextTahap[item.tahap] }}
                  </button>
                </td>
              </tr>
              <tr v-if="!filtered.length">
                <td colspan="5" class="py-8 text-center text-slate-400 text-sm font-black">
...
.lanjut-btn { background: #eff6ff; border-color: #bfdbfe; color: #2563eb; }
.lanjut-btn:hover:not(:disabled) { background: #dbeafe; border-color: #93c5fd; }
