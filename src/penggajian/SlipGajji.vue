...
/**
 * ============================================================
 *  FILE: src/views/SlipGaji.vue
 *  TUJUAN: Tampilan slip gaji pegawai - data dari API.
 *
 *  ✅ UNTUK BACKEND: File ini TIDAK perlu disentuh. Ubah endpoint
 *  di useSlipGaji composable.
 *  ✅ UNTUK FRONTEND: Hanya ubah tampilan (HTML/CSS) di sini.
 * ============================================================
 */

import {
  FileText, Download, CheckCircle2, CalendarDays,
  Search, Loader2, Check, AlertCircle, RefreshCw, Eye,
} from 'lucide-vue-next'

import AppLayout from '@/components/AppLayout.vue'
import { useSlipGaji } from '@/composables/useSlipGaji'
...
const statIcons  = [FileText, CheckCircle2, Download, CalendarDays]
const statColors = [
...
<template>
  <AppLayout breadcrumb="Penggajian" title="Slip Gaji">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center min-h-[400px] gap-4">
      ...
      <p class="text-slate-500 text-sm font-black tracking-wide">Memuat data slip gaji...</p>
    </div>
      ...
      <div class="card rounded-[24px] p-5 sm:p-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <div>
            ...
            <p class="card-sub">Periode {{ periode }} · {{ filtered.length }} pegawai</p>
          </div>
          ...
          <div class="relative">
            ...
            <input
              ...
              class="h-9 w-52 rounded-xl border-2 border-slate-200 bg-white pl-9 pr-3 text-sm font-bold text-slate-700 outline-none focus:border-sky-400 transition-all placeholder:font-normal placeholder:text-slate-400"
            />
          </div>
        </div>
          ...
          <table class="w-full min-w-[640px]">
            ...
            <tbody>
              ...
              >
                ...
                <td class="td text-sm font-bold text-slate-600">{{ periode }}</td>
                <td class="td text-sm font-black text-slate-800 tabular-nums">{{ rupiah(item.gajiBersih) }}</td>
                <td class="td">
                  <span
                    class="badge"
                    :class="item.status === 'Diunduh' ? 'badge-green' : 'badge-amber'"
                  >{{ item.status }}</span>
                </td>
                <td class="td">
                  <div class="flex items-center gap-2 justify-end">
                    <button class="icon-btn" title="Lihat slip">
                      <Eye :size="14" />
                    </button>
                    <button
                      @click="unduhSlip(item)"
                      :disabled="downloading === item.id"
                      class="unduh-btn flex items-center gap-1.5 px-3.5 py-2 rounded-xl font-black text-xs transition-all duration-200 hover:-translate-y-0.5 active:scale-95 border-2 disabled:opacity-60"
                    >
                      <Loader2 v-if="downloading === item.id" :size="13" class="animate-spin" />
                      <Check v-else-if="item.status === 'Diunduh'" :size="13" />
                      <Download v-else :size="13" />
                      {{ item.status === 'Diunduh' ? 'Unduh Lagi' : 'Unduh' }}
                    </button>
                  </div>
                </td>
              </tr>
...
.unduh-btn { background: #eff6ff; border-color: #bfdbfe; color: #2563eb; }
.unduh-btn:hover:not(:disabled) { background: #dbeafe; border-color: #93c5fd; }
