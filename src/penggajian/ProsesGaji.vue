...
/**
 * ============================================================
 *  FILE: src/views/ProsesGaji.vue
 *  TUJUAN: Tampilan proses penggajian - data dari API, bukan hardcode.
 *
 *  ✅ UNTUK BACKEND:
 ...
 *  - Semua data & logic ada di useProsesGaji composable
 *  - Hanya ubah tampilan (HTML/CSS) di sini
 * ============================================================
 */

import {
  Users, ClipboardCheck, Clock3, Wallet,
  Search, ChevronDown, PlayCircle, Loader2, Check,
  AlertCircle, RefreshCw, CalendarDays,
} from 'lucide-vue-next'

import AppLayout from '@/components/AppLayout.vue'
import { useProsesGaji } from '@/composables/useProsesGaji'
...
const statIcons  = [Users, ClipboardCheck, Clock3, Wallet]
const statColors = [
...
<template>
  <AppLayout breadcrumb="Penggajian" title="Proses Gaji">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center min-h-[400px] gap-4">
      ...
      <p class="text-slate-500 text-sm font-black tracking-wide">Memuat data proses gaji...</p>
    </div>
    ...
    <div v-else class="space-y-5 pb-6 animate-fadein">
      ...
      <!-- ── Header periode + Proses Semua ── -->
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div class="flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-white border-2 border-slate-200 w-fit">
          <CalendarDays :size="16" class="text-slate-400" />
          <span class="text-sm font-black text-slate-700">Periode: {{ periode }}</span>
        </div>
        <button
          @click="prosesSemua"
          class="proses-btn flex items-center justify-center gap-2.5 px-5 py-2.5 rounded-2xl font-black text-sm text-white transition-all duration-300 hover:-translate-y-0.5 active:scale-95 shadow-lg"
        >
          <PlayCircle :size="17" />
          Proses Semua Gaji
        </button>
      ...
      <div class="card rounded-[24px] p-5 sm:p-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <div>
            <h3 class="card-title">Daftar Penggajian Pegawai</h3>
          ...
          <table class="w-full min-w-[720px]">
            <thead>
              <tr>
                ...
                <th class="th">Gaji Pokok</th>
                <th class="th">Tunjangan</th>
                <th class="th">Potongan</th>
                <th class="th">Total Gaji</th>
            ...
            <tbody>
              ...
              >
                ...
                <td class="td text-sm font-bold text-slate-600 tabular-nums">{{ rupiah(item.gajiPokok) }}</td>
                <td class="td text-sm font-bold text-emerald-600 tabular-nums">+{{ rupiah(item.tunjangan) }}</td>
                <td class="td text-sm font-bold text-red-500 tabular-nums">-{{ rupiah(item.potongan) }}</td>
                <td class="td text-sm font-black text-slate-800 tabular-nums">{{ rupiah(totalGaji(item)) }}</td>
                <td class="td">
                  <span
                    class="badge"
                    :class="item.status === 'Diproses' ? 'badge-green' : 'badge-amber'"
                  >{{ item.status }}</span>
                </td>
                <td class="td">
                  <button
                    v-if="item.status === 'Menunggu'"
                    @click="prosesGaji(item)"
                    :disabled="processing === item.id"
                    class="proses-row-btn flex items-center gap-1.5 px-3.5 py-2 rounded-xl font-black text-xs transition-all duration-200 hover:-translate-y-0.5 active:scale-95 border-2 disabled:opacity-60"
                  >
                    <Loader2 v-if="processing === item.id" :size="13" class="animate-spin" />
              ...
              <tr v-if="!filtered.length">
                <td colspan="7" class="py-8 text-center text-slate-400 text-sm font-black">
...
.avatar {
  ...
  font-size: 12px; font-weight: 900; color: #0369a1; flex-shrink: 0;
}
...
.proses-btn:hover { box-shadow: 0 6px 28px rgba(14,165,233,0.45); }

.proses-row-btn {
  background: #eff6ff; border-color: #bfdbfe; color: #2563eb;
}
.proses-row-btn:hover:not(:disabled) { background: #dbeafe; border-color: #93c5fd; }
