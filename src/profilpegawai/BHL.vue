...
/**
 * ============================================================
 *  FILE: src/profilpegawai/BHL.vue
...
import {
  ...
  Search, ChevronDown, Loader2, AlertCircle, RefreshCw,
} from 'lucide-vue-next'

import AppLayout from '@/components/AppLayout.vue'
import { useBHL } from '@/composables/useBHL'
...
const statIcons     = [Users, UserCheck, UserX, Wallet]
const statIconColor = ['#0ea5e9', '#10b981', '#ef4444', '#f59e0b']
const statBg        = [
...
const statusColor = (status) => {
  const map = {
    ...
    'Absen': 'bg-red-50     text-red-600     border border-red-200',
  }
...
<template>
  <AppLayout breadcrumb="Profil Pegawai" title="BHL (Buruh Harian Lepas)">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center min-h-[400px] gap-4">
      ...
      <p class="text-slate-600 text-sm font-black tracking-wide">Memuat data BHL...</p>
    </div>
    ...
    <div v-else class="space-y-5 pb-6 page-enter">
      ...
      <div class="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
        ...
        >
          ...
          <div class="relative z-10">
            ...
            <h2
              class="text-2xl sm:text-3xl font-black tabular-nums text-slate-800 leading-none num-pop"
      ...
      <div class="card-glass rounded-[24px] p-5 sm:p-6 slide-up" style="animation-delay:150ms">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <div>
            <h3 class="text-base font-black text-slate-800 tracking-tight">Data BHL</h3>
            <p class="text-[11px] font-black text-slate-400 mt-0.5 uppercase tracking-[0.1em]">
              <span class="text-sky-600">{{ filtered.length }}</span> pekerja ditemukan
            </p>
          </div>
        ...
        <div class="overflow-x-auto -mx-1">
          <table class="w-full min-w-[760px]">
            <thead>
              <tr class="text-left">
                <th v-for="h in ['Nama Pekerja','Divisi','Tgl Daftar','Hadir Bulan Ini','Upah Harian','Status Hari Ini']"
                    :key="h"
              ...
              >
                <td class="py-3.5 px-2">
                  <div class="flex items-center gap-2.5">
                    <div class="w-8 h-8 rounded-xl bg-gradient-to-br from-amber-100 to-orange-100
                                border-2 border-amber-200 flex items-center justify-center
                                text-xs font-black text-amber-700 shrink-0">
                      {{ item.nama.charAt(0) }}
                    </div>
                ...
                <td class="py-3.5 px-2 text-sm font-bold text-slate-600 whitespace-nowrap">{{ item.tanggalDaftar }}</td>
                <td class="py-3.5 px-2 text-sm font-black text-slate-800 tabular-nums whitespace-nowrap">{{ item.hadirBulanIni }} hari</td>
                <td class="py-3.5 px-2 text-sm font-black text-slate-800 tabular-nums whitespace-nowrap">
                  Rp {{ item.upahHarian.toLocaleString('id-ID') }}
                </td>
                <td class="py-3.5 px-2">
                  <span class="px-2.5 py-1 rounded-lg text-xs font-black whitespace-nowrap" :class="statusColor(item.statusHariIni)">
                    {{ item.statusHariIni }}
                  </span>
                </td>
              </tr>
