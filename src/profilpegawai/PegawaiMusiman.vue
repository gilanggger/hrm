...
/**
 * ============================================================
 *  FILE: src/profilpegawai/PegawaiMusiman.vue
 * ============================================================
 */

import {
  Users, UserCheck, CalendarRange, Timer,
  Search, ChevronDown, Loader2, AlertCircle, RefreshCw, Phone,
...
const statIcons     = [Users, UserCheck, Timer, CalendarRange]
const statIconColor = ['#8b5cf6', '#10b981', '#f59e0b', '#0ea5e9']
const statBg        = [
...
const statusColor = (status) => {
  const map = {
    ...
    'Selesai Kontrak': 'bg-slate-100  text-slate-500   border border-slate-200',
  }
...
<template>
  <AppLayout breadcrumb="Profil Pegawai" title="Pegawai Musiman">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center min-h-[400px] gap-4">
      ...
      <p class="text-slate-600 text-sm font-black tracking-wide">Memuat data pegawai musiman...</p>
    </div>
    ...
    <div v-else class="space-y-5 pb-6 page-enter">
      ...
      <!-- ════════════════════════════════════════
           INFO MUSIM GILING AKTIF
      ════════════════════════════════════════ -->
      <div class="card-glass rounded-[24px] p-5 flex items-center gap-4 slide-up" style="animation-delay:120ms">
        <div class="w-11 h-11 rounded-xl bg-violet-50 border-2 border-violet-200 flex items-center justify-center shrink-0">
          <CalendarRange :size="20" class="text-violet-600" />
        </div>
        <div>
          <p class="text-sm font-black text-slate-800">Musim Giling 2026</p>
          <p class="text-[11px] font-black text-slate-400 uppercase tracking-[0.1em] mt-0.5">
            Kontrak pegawai musiman mengikuti periode musim giling berjalan
          </p>
        </div>
      </div>

      <!-- ════════════════════════════════════════
           TABEL PEGAWAI MUSIMAN
      ════════════════════════════════════════ -->
      <div class="card-glass rounded-[24px] p-5 sm:p-6 slide-up" style="animation-delay:200ms">

        <!-- Header + Filter -->
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <div>
            <h3 class="text-base font-black text-slate-800 tracking-tight">Data Pegawai Musiman</h3>
        ...
        <div class="overflow-x-auto -mx-1">
          <table class="w-full min-w-[820px]">
            <thead>
              <tr class="text-left">
                <th v-for="h in ['NIK','Nama Pegawai','Divisi','Periode','Tgl Mulai','Tgl Selesai','Kontak','Status']"
                    :key="h"
              ...
              >
                ...
                <td class="py-3.5 px-2">
                  <div class="flex items-center gap-2.5">
                    <div class="w-8 h-8 rounded-xl bg-gradient-to-br from-violet-100 to-purple-100
                                border-2 border-violet-200 flex items-center justify-center
                                text-xs font-black text-violet-700 shrink-0">
                      {{ item.nama.charAt(0) }}
                    </div>
                ...
                <td class="py-3.5 px-2">
                  <span class="px-2.5 py-1 rounded-lg bg-violet-50 border-2 border-violet-200 text-violet-700 text-xs font-black whitespace-nowrap">
                    {{ item.periodeGiling }}
                  </span>
                </td>
                <td class="py-3.5 px-2 text-sm font-bold text-slate-600 whitespace-nowrap">{{ item.tanggalMulai }}</td>
                <td class="py-3.5 px-2 text-sm font-bold text-slate-600 whitespace-nowrap">{{ item.tanggalSelesai }}</td>
                <td class="py-3.5 px-2">
