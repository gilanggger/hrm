...
/**
 * ============================================================
 *  FILE: src/keselamatankerja/MedicalCheckUp.vue
 *  TUJUAN: Tampilan medical check up pegawai - data dari API.
 *
 *  ✅ UNTUK BACKEND: File ini TIDAK perlu disentuh. Ubah endpoint
 *  di useMedicalCheckUp composable.
 *  ✅ UNTUK FRONTEND: Hanya ubah tampilan (HTML/CSS) di sini.
 * ============================================================
 */

import {
  Users, HeartPulse, AlertTriangle, ClipboardList,
  Search, ChevronDown, CalendarPlus, Loader2,
  AlertCircle, RefreshCw,
} from 'lucide-vue-next'

import AppLayout from '@/components/AppLayout.vue'
import { useMedicalCheckUp } from '@/composables/useMedicalCheckUp'
...
const statIcons  = [Users, HeartPulse, ClipboardList, AlertTriangle]
const statColors = [
...
const hasilBadge = {
  Sehat: 'badge-green',
  'Perlu Perhatian': 'badge-amber',
  'Tidak Fit': 'badge-red',
  'Belum MCU': 'badge-slate',
}
</script>

<template>
  <AppLayout breadcrumb="Keselamatan Kerja" title="Medical Check Up">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center min-h-[400px] gap-4">
      ...
      <p class="text-slate-500 text-sm font-black tracking-wide">Memuat data medical check up...</p>
    </div>
    ...
    <div v-else class="space-y-5 pb-6 animate-fadein">
      ...
      <div class="card rounded-[24px] p-5 sm:p-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <div>
            <h3 class="card-title">Riwayat Medical Check Up</h3>
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <div class="relative">
              ...
              >
                <option v-for="s in hasilList" :key="s">{{ s }}</option>
              </select>
          ...
          <table class="w-full min-w-[640px]">
            ...
            <tbody>
              ...
              >
                ...
                <td class="td">
                  <span class="badge" :class="hasilBadge[item.hasil]">{{ item.hasil }}</span>
                </td>
                <td class="td">
                  <button
                    v-if="item.hasil === 'Belum MCU'"
                    @click="jadwalkanMcu(item)"
                    :disabled="scheduling === item.id"
                    class="jadwal-btn flex items-center gap-1.5 px-3.5 py-2 rounded-xl font-black text-xs transition-all duration-200 hover:-translate-y-0.5 active:scale-95 border-2 disabled:opacity-60"
                  >
                    <Loader2 v-if="scheduling === item.id" :size="13" class="animate-spin" />
                    <CalendarPlus v-else :size="13" />
                    Jadwalkan
                  </button>
                </td>
              </tr>
...
.avatar {
  ...
  font-size: 12px; font-weight: 900; color: #15803d; flex-shrink: 0;
}
...
.jadwal-btn { background: #eff6ff; border-color: #bfdbfe; color: #2563eb; }
.jadwal-btn:hover:not(:disabled) { background: #dbeafe; border-color: #93c5fd; }
