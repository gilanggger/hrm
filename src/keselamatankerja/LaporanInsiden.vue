...
/**
 * ============================================================
 *  FILE: src/keselamatankerja/LaporanInsiden.vue
 *  TUJUAN: Tampilan laporan insiden K3 - data dari API.
 *
 *  ✅ UNTUK BACKEND: File ini TIDAK perlu disentuh. Ubah endpoint
 *  di useLaporanInsiden composable.
 *  ✅ UNTUK FRONTEND: Hanya ubah tampilan (HTML/CSS) di sini.
 * ============================================================
 */

import {
  ShieldAlert, TriangleAlert, AlertOctagon, CheckCircle2,
  Search, ChevronDown, MapPin, Loader2, Check,
  AlertCircle, RefreshCw, Plus,
} from 'lucide-vue-next'

import AppLayout from '@/components/AppLayout.vue'
import { useLaporanInsiden } from '@/composables/useLaporanInsiden'
...
const statIcons  = [ShieldAlert, AlertOctagon, TriangleAlert, CheckCircle2]
const statColors = [
...
<template>
  <AppLayout breadcrumb="Keselamatan Kerja" title="Laporan Insiden">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center min-h-[400px] gap-4">
      ...
      <p class="text-slate-500 text-sm font-black tracking-wide">Memuat data insiden...</p>
    </div>
    ...
    <div v-else class="space-y-5 pb-6 animate-fadein">
      ...
      <!-- ── Tombol Lapor Insiden ── -->
      <div class="flex justify-end">
        <button
          class="lapor-btn flex items-center gap-2.5 px-5 py-2.5 rounded-2xl font-black text-sm text-white transition-all duration-300 hover:-translate-y-0.5 active:scale-95 shadow-lg"
        >
          <Plus :size="17" />
          Lapor Insiden Baru
        </button>
      ...
      <div class="card rounded-[24px] p-5 sm:p-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <div>
            <h3 class="card-title">Riwayat Insiden Kerja</h3>
            <p class="card-sub">{{ filtered.length }} insiden ditemukan</p>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <div class="relative">
              ...
              >
                <option v-for="s in tingkatList" :key="s">{{ s }}</option>
              </select>
        ...
        <div class="overflow-x-auto -mx-1">
          <table class="w-full min-w-[760px]">
            ...
            <tbody>
              ...
              >
                <td class="td text-sm font-bold text-slate-600 tabular-nums whitespace-nowrap">{{ item.tanggal }}</td>
                <td class="td">
                ...
                <td class="td text-sm font-bold text-slate-700">{{ item.jenis }}</td>
                <td class="td">
                  <span class="badge" :class="tingkatBadge[item.tingkat]">{{ item.tingkat }}</span>
                </td>
                <td class="td">
                  <span class="badge" :class="item.status === 'Ditutup' ? 'badge-green' : 'badge-sky'">{{ item.status }}</span>
                </td>
                <td class="td">
                  <button
                    v-if="item.status === 'Investigasi'"
                    @click="tutupInsiden(item)"
                    :disabled="updating === item.id"
                    class="tutup-btn flex items-center gap-1.5 px-3.5 py-2 rounded-xl font-black text-xs transition-all duration-200 hover:-translate-y-0.5 active:scale-95 border-2 disabled:opacity-60"
                  >
...
.avatar {
  ...
  font-size: 12px; font-weight: 900; color: #b91c1c; flex-shrink: 0;
}
...
.badge-sky   { background: #eff6ff; border-color: #bfdbfe; color: #2563eb; }

.lapor-btn {
  background: linear-gradient(135deg, #ef4444, #f97316);
  box-shadow: 0 4px 20px rgba(239,68,68,0.35);
}
.lapor-btn:hover { box-shadow: 0 6px 28px rgba(239,68,68,0.45); }

.tutup-btn { background: #f0fdf4; border-color: #bbf7d0; color: #16a34a; }
.tutup-btn:hover:not(:disabled) { background: #dcfce7; border-color: #86efac; }
